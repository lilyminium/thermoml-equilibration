...
@click.option(
    ...
    "-t",
    default=0.5,
    help="The threshold to consider a system equilibrated",
)
def main(
    ...
    # get properties with equilibrated fraction > threshold
    subset = [
        x for x in equilibrated_fraction
        if x["equilibrated_fraction"] > threshold
    ]
    subset.sort(
        key=lambda x: x["equilibrated_fraction"], reverse=True
    )

    print(f"Found {len(subset)} equilibrated systems with fraction > {threshold}")

    # get molecules involved
    unique_molecules = collections.defaultdict(list)
    for item in subset:
        for smi in item["components"]:
            if smi == "O": # ignore water
                continue
    ...
    print(f"Found {len(unique_molecules)} unique molecules > {threshold}")

    output_directory = pathlib.Path(output_directory)
    output_directory.mkdir(exist_ok=True, parents=True)
    
    output_smiles = output_directory / "outlier_molecules.json"
    with open(output_smiles, "w") as f:
        json.dump(list(unique_molecules), f, indent=4)
    print(f"Saved to {output_smiles}")
